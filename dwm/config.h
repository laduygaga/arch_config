...
static const char *fonts[]          = { "mono:size=9", "Symbola:size=7" };
...
static const Rule rules[] = {
    ...
    { NULL,                 NULL,       "S-Project", 0,          1,          1,          1,          -1 },                   // gab208
...
static Key keys[] = {
    ...
    { MODKEY,               XK_F12,                     spawn,  SHCMD("filepath=~/Pictures/ScreenShots/$(date +'%b%d_%H%M%S').png && scrot -s \"$filepath\" && tesseract \"$filepath\" - -l eng quiet | xclip -sel clip") },
    { MODKEY|ShiftMask,     XK_F12,                     spawn,  SHCMD("lang=$(echo \"vie\nchi_sim\njpn\" | dmenu -l 3) && filepath=~/Pictures/ScreenShots/$(date +'%b%d_%H%M%S').png && scrot -s \"$filepath\" && tesseract \"$filepath\" - -l \"$lang\" quiet | xclip -sel clip") },
