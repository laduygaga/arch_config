...
static const Rule rules[] = {
	...
	{ "hearthstone.exe",    NULL,       NULL,           1 << 5,     0,          0,          1,          -1 },
...
static Key keys[] = {
	...
	{ MODKEY,               XK_F5,                      spawn,  SHCMD("brightnessctl s 5%-") },
	...
	{ 0,                    XK_Insert,                   spawn,  SHCMD("scrot ~/Pictures/ScreenShots/%b%d:%H%M%S.png -e 'xclip -selection clipboard -target image/png -i $f'") },
	{ MODKEY,               XK_Insert,                   spawn,  SHCMD("scrot -s ~/Pictures/ScreenShots/%b%d:%H%M%S.png -e 'xclip -selection clipboard -target image/png -i $f'") },
    { MODKEY,               XK_F12,                     spawn,  SHCMD("filepath=~/Pictures/ScreenShots/$(date +'%b%d_%H%M%S').png && scrot -s \"$filepath\" && tesseract \"$filepath\" - -l eng quiet | xsel -b") },
    { MODKEY|ShiftMask,     XK_F12,                     spawn,  SHCMD("lang=$(echo \"vie\nchi_sim\njpn\" | dmenu -l 3) && filepath=~/Pictures/ScreenShots/$(date +'%b%d_%H%M%S').png && scrot -s \"$filepath\" && tesseract \"$filepath\" - -l \"$lang\" quiet | xsel -b") },
	{ MODKEY,               XK_Delete,                  spawn,  SHCMD("turnoff_screen.sh") },
	{ MODKEY,               XK_r,                       spawn,  SHCMD("st -e lf") },
	// { MODKEY,               XK_g,                       spawn,  SHCMD("goldendict") },
	{ MODKEY|ControlMask,   XK_g,                       spawn,  SHCMD("token_generator.sh") },
	...
	{ MODKEY|ShiftMask,     XK_period,                  spawn,  SHCMD("xterm -geometry 50x20+0+590 -e trans :en --shell") },
